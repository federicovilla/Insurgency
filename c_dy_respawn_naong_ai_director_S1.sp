 ...
 */
#pragma dynamic 32768	// Increase heap size : 32768 cells -> a cell is 4byte, then 32768*4=131072 = 128kB
#pragma semicolon 1
#include <sourcemod>
#include <sdktools>
#include <sdkhooks>
...
public Action:Timer_WaveSpawning(Handle:Timer)
{
	...
	if (g_respawn_mode_team_sec)
	{
			...
			g_secWave_Timer <= 3)
		{
			...
			//PrintHintTextToAll(textToPrint); //note
			//PrintToChatAll(textToPrintChat); //note
		}

		//Reset Wave SEC timer and announce respawn
		if (g_secWave_Timer <= 0)
		{
			//PrintHintTextToAll("[INTEL]Team Reinforced!"); // note
			//PrintToChatAll("[INTEL]Team Reinforced!");    // note
...
{
	if(client > MaxClients || client <= 0) return; // ADDED										   
...
public Action:FireScreamCheckTimer(Handle:timer, any:entity)
{
	...
	new owner;
	if (IsValidEntity(entity) && entity > 0 && HasEntProp(entity, Prop_Send, "m_hOwnerEntity")) // ADDED
	{
